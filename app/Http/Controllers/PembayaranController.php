...
class PembayaranController extends Controller
{
    ...
    public function kwitansi($id)
    {
        ...
        $data['nonRaciks']      = PendaftaranResep::with('barang')->where('jenis', 'non racik')
                                ->where('pendaftaran_id', $data['nomorAntrian']->pendaftaran_id)
                                ->get();
        $data['obatRacik']      = \App\Models\PendaftaranObatRacik::with('detail.barang')->where('pendaftaran_id', $data['nomorAntrian']->pendaftaran_id)
                                ->where('poliklinik_id',Auth::user()->poliklinik_id)->get()
                                ->get();
